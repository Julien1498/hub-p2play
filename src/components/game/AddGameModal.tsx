...
export function AddGameModal({ isOpen, onClose, onGameAdded }: AddGameModalProps) {
  ...
  const handleAddGame = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Échec de l\'ajout du jeu. Vérifiez le dépôt et réessayez.');
      setLoading(false);
      setProgressMsg(null);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-2xl relative space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-zinc-850 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-zinc-100">Ajouter un jeu GitHub Live</h3>
              <p className="text-xs text-zinc-400">Chargez n'importe quel jeu compatible P2Play sans recompiler</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  key={ex.slug}
                  type="button"
                  onClick={() => handleExampleClick(ex.slug)}
                  disabled={loading}
          ...
          {successMeta && (
            <div className="p-4 bg-emerald-950/30 border border-emerald-900/50 rounded-2xl flex items-center gap-3 text-emerald-300">
              ...
              <div className="text-xs">
                ...
                <span>"{successMeta.name}" ajouté avec succès au Hub.</span>
              </div>
            </div>
          )}
